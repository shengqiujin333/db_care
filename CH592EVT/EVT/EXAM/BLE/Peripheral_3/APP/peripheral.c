...
#define DEFAULT_DESIRED_MIN_CONN_INTERVAL    1500
...
#define DEFAULT_DESIRED_CONN_TIMEOUT         100*30

// Company Identifier: WCH
#define WCH_COMPANY_ID                       0x08D9
...
static uint8_t scanRspData[] = {
    // complete name
    0x04, // length of this data (new length: 1 byte for length + 1 byte for type + 3 bytes for "xxx")
...
// GAP GATT Attributes
static uint8_t attDeviceName[GAP_DEVICE_NAME_LEN] = "JYN Peripheral";
...
void Peripheral_Init()
{
    ...
    {
        uint32_t passkey = 240730; // passkey "000000"
...
//    DevInfo_AddService();                        // Device Information Service
    ...
    // Setup the SimpleProfile Characteristic Values
    {
        ...
        uint8_t charValue2[SIMPLEPROFILE_CHAR2_LEN] = {4};
...
//        attExchangeMTUReq_t tempmtu;
//        tempmtu.clientRxMTU = 256;
//        PRINT("EXchange mtu:%d %d %d\r\n",peripheralConnList.connHandle,Peripheral_TaskID,GATT_ExchangeMTU( peripheralConnList.connHandle,&tempmtu, Peripheral_TaskID ));


        PRINT("GET MTU:%d\r\n",ATT_GetMTU(connInterval));
    }
    else
    {
        PRINT("ERR..\n");
...
static void performPeriodicTask(void)
{
    ...
    uint8_t notiData[SIMPLEPROFILE_CHAR4_LEN] = {0x88,0x66};
    notiData[0] = 0;
    notiData[1] = 1;
...
static void simpleProfileChangeCB(uint8_t paramID, uint8_t *pValue, uint16_t len)
{
    switch(paramID)
    {
        ...
        case SIMPLEPROFILE_CHAR3:
        {
            ...
            tmos_memcpy(newValue, pValue, len);
            SimpleProfile_SetParameter(SIMPLEPROFILE_CHAR3, SIMPLEPROFILE_CHAR3_LEN, pValue);
            PRINT("profile ChangeCB CHAR3..%d,%d\n",len,peripheralMTU);
//            uint8_t i;
//            for(i = 0; i < len;i++){
//                PRINT("%x ",pValue[i]);
//            }
