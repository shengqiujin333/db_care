...
#line 1 "..\\..\\..\\..\\Libraries\\inc\\cw32l010_systick.h"
...
#line 39 "..\\..\\..\\..\\Libraries\\inc\\cw32l010_systick.h"
#line 40 "..\\..\\..\\..\\Libraries\\inc\\cw32l010_systick.h"
...
volatile uint8_t gKeyStatus = 0;   
volatile uint8_t gLPTIMWakeUpIrq = 0;   
volatile uint8_t gIWDTWakeUpIrq = 0;   
volatile uint8_t gRTCWakeUpIrq = 0;   
volatile uint8_t gUARTWakeUpIrq = 0;   
...
int main(void)
{
    ...
    SYSCTRL_HSI_Enable(((uint32_t)(12UL << (11UL))));
    ...
    GPIO_InitStructure.IT = (0x82);
    GPIO_InitStructure.Mode = (0x11);
    GPIO_InitStructure.Pins = ((uint16_t)0x0040);
    
    GPIO_Init(((GPIO_TypeDef*) 0x48000000UL), &GPIO_InitStructure);
    __NVIC_EnableIRQ(GPIOA_IRQn);
    ...
    while(1)
    {
        if (gKeyStatus)
        {
            gKeyStatus = 0;
            SysTickDelay(100);    
            if (! !(((GPIO_TypeDef*) 0x48000000UL)->IDR & (((uint32_t)1u) << 6)) == 0)    
            {
                TestState++;     
            }
            else
        ...
        switch (TestState)
        {
            ...
            case 0:    
                ...
                ((SYSCTRL_TypeDef*) 0x40004000UL)->APBEN1 = 0x5a5a0000;
                ((SYSCTRL_TypeDef*) 0x40004000UL)->APBEN2 = 0x5a5a0000;
                ((SYSCTRL_TypeDef*) 0x40004000UL)->AHBEN  = 0x5a5a0000;
                
                SYSCTRL_AHBPeriphClk_Enable((((uint32_t)1u) << 4), ENABLE);                
                break;
            case 1:    
                if (!(IsPeriphRun & 1UL))
                {
                ...
                break;
            case 2:    
                if (!(IsPeriphRun & (1UL << 2)))
                {
                    
                    IWDT_Stop();
                    SYSCTRL_APBPeriphReset2((((uint32_t)1u) << 4), ENABLE);
                    SYSCTRL_APBPeriphReset2((((uint32_t)1u) << 4), DISABLE);
                    SYSCTRL_APBPeriphClk_Enable2((((uint32_t)1u) << 4), DISABLE);
                    IsPeriphRun &= ~1UL;
                ...
                break;
            case 3:    
                if (!(IsPeriphRun & (1UL << 3)))
                {
                    
                    LPTIM_Cmd(DISABLE);
                    SYSCTRL_APBPeriphClk_Enable2((((uint32_t)1u) << 7), DISABLE);
                    IsPeriphRun &= ~(1UL << 2);
                ...
                break;
            case 4:    
                if (!(IsPeriphRun & (1UL << 1)))
                {
                    
                    SYSCTRL_APBPeriphReset1((((uint32_t)1u) << 3), ENABLE);
                    SYSCTRL_APBPeriphReset1((((uint32_t)1u) << 3), DISABLE);
                    SYSCTRL_APBPeriphClk_Enable1((((uint32_t)1u) << 3), DISABLE);
                    IsPeriphRun &= ~(1UL << 3);
 ...
 void RTC_Configuration(void)
 {
     ...
     RTC_InitStruct.DateStruct.Month = ((uint8_t)0x01);
     RTC_InitStruct.DateStruct.Year = 24;
     RTC_InitStruct.DateStruct.Week = ((uint8_t)0x00);
     RTC_InitStruct.RTC_ClockSource = (2UL << 8);
     RTC_InitStruct.TimeStruct.AMPM = (0UL);
     RTC_InitStruct.TimeStruct.H24 = (0UL);
     ...
     RTC_SetInterval((0x02));    
     RTC_ITConfig((1UL << 6), ENABLE);
     RTC_ClearITPendingBit(((1UL)|(1UL << 1)|(1UL << 2)|(1UL << 3)|(1UL << 4)|(1UL << 6)));
...
void LPTIM_Configuration(void)
{
    ...
    LPTIM_InitStruct.LPTIM_ClockSource = ((uint32_t)0x00000000);
    LPTIM_InitStruct.LPTIM_CounterMode = ((uint32_t)0x00000000);
    LPTIM_InitStruct.LPTIM_Period = 32800-1;
    LPTIM_InitStruct.LPTIM_Prescaler = (((uint32_t)0x00000001) << 9);
    LPTIM_Init(&LPTIM_InitStruct);
    
    SYSCTRL_LSI_Enable();    
    LPTIM_InternalClockConfig((((uint32_t)0x00000003) << 25));

    LPTIM_ITConfig((((uint32_t)0x00000001) << 1), ENABLE);
    ((LPTIM_TypeDef*) 0x40006000UL)->ICR = 0x00;
	__NVIC_EnableIRQ(LPTIM_IRQn);
...
void UART1_Configuration(void)
{
    ...
    GPIO_InitStructure.Pins = ((uint16_t)0x0020);
    GPIO_InitStructure.Mode = (0x11);
    GPIO_Init(((GPIO_TypeDef*) 0x48000000UL), &GPIO_InitStructure);   
    
    (((GPIO_TypeDef*) 0x48000000UL)->AFRL_f . AFR5 = 1);
    
    UART_InitStructure.UART_BaudRate = 9600;
    UART_InitStructure.UART_Over = ((uint16_t)0x0600);    
    UART_InitStructure.UART_Source = ((uint16_t)0x3000);
    UART_InitStructure.UART_UclkFreq = 32800;
    UART_InitStructure.UART_StartBit = ((uint16_t)0x0000);
    UART_InitStructure.UART_StopBits = ((uint16_t)0x0000);
    UART_InitStructure.UART_Parity = ((uint16_t)0x0000) ;
    UART_InitStructure.UART_HardwareFlowControl = ((uint16_t)0x0000);
    UART_InitStructure.UART_Mode = ((uint16_t)0x0002);
    UART_Init(((UART_TypeDef*) 0x40000C00UL), &UART_InitStructure); 
    
    UART_ITConfig(((UART_TypeDef*) 0x40000C00UL), ((uint16_t)0x0004), ENABLE);
    UART_ClearFlag(((UART_TypeDef*) 0x40000C00UL), ((uint16_t)0x0004));
...
void DebugUART_Configuration(uint32_t UclkFreq)
{
    ...
    GPIO_InitStructure.Pins = ((uint16_t)0x0020);
    GPIO_InitStructure.Mode = (0x20);
    GPIO_Init(((GPIO_TypeDef*) 0x48000100UL), &GPIO_InitStructure);
    ...
    (((GPIO_TypeDef*) 0x48000100UL)->AFRL_f . AFR5 = 1);
    (((GPIO_TypeDef*) 0x48000100UL)->AFRL_f . AFR6 = 1);    

    UART_InitStructure.UART_BaudRate = 9600;
    UART_InitStructure.UART_Over = ((uint16_t)0x0000);
    UART_InitStructure.UART_Source = ((uint16_t)0x0000);
    UART_InitStructure.UART_UclkFreq = UclkFreq;
    UART_InitStructure.UART_StartBit = ((uint16_t)0x0000);
    UART_InitStructure.UART_StopBits = ((uint16_t)0x0000);
    UART_InitStructure.UART_Parity = ((uint16_t)0x0000) ;
    UART_InitStructure.UART_HardwareFlowControl = ((uint16_t)0x0000);
    UART_InitStructure.UART_Mode = ((uint16_t)0x0002) | ((uint16_t)0x0001);
    UART_Init(((UART_TypeDef*) 0x40001000UL), &UART_InitStructure);
}

void DebugUART_Close(void)
{
    SYSCTRL_AHBPeriphReset((((uint32_t)1u) << 5), ENABLE);
    SYSCTRL_AHBPeriphReset((((uint32_t)1u) << 5), DISABLE);
    SYSCTRL_APBPeriphReset1((((uint32_t)1u) << 4), ENABLE);
    SYSCTRL_APBPeriphReset1((((uint32_t)1u) << 4), DISABLE);
    
    SYSCTRL_AHBPeriphClk_Enable((((uint32_t)1u) << 5), DISABLE);
    SYSCTRL_APBPeriphClk_Enable1((((uint32_t)1u) << 4), DISABLE);
}
...
{
    UART_SendData_8bit(((UART_TypeDef*) 0x40001000UL), (uint8_t)ch);

    while (UART_GetFlagStatus(((UART_TypeDef*) 0x40001000UL), ((uint16_t)0x0001)) == RESET);
...
size_t __write(int handle, const unsigned char * buffer, size_t size)
{
    ...
    for ( ; size != 0; --size)
    {
        UART_SendData_8bit(((UART_TypeDef*) 0x40001000UL), *buffer++);
        while (UART_GetFlagStatus(((UART_TypeDef*) 0x40001000UL), ((uint16_t)0x0001)) == RESET);
        ++nChars;
    }
