...
int main(void)
{
    ...
    ADC_InitStructure.ADC_SQREns = ADC_SqrEns0to1;     // 单通道
    ...
    while (1)
    {
        ...
        valueAdc[0] = ADC_GetConversionValue(0);
		valueAdc[1] = ADC_GetConversionValue(1);
		BGR_1_2V_value = *((uint16_t*)	0x001007D2);
		MCU_VDD = (4095 * BGR_1_2V_value * 0.001f )/ valueAdc[0];
		TS_MCU = ADC_GetTs(MCU_VDD,valueAdc[1]);//不一定是3.3V，芯片供电电压是多少伏，就填多少伏
        ...
        if (gFlagIrq & ADC_ISR_EOS_Msk)
        {
			...
			MCU_VDD = (4.095f * BGR_1_2V_value)/ valueAdc[0]; //计算出VDD
			TS_MCU = ADC_GetTs(MCU_VDD,valueAdc[1]);//不一定是3.3V，芯片供电电压是多少伏，就填多少伏
            gFlagIrq = 0;
#ifndef SGL_CONTINUOUS_MODE_EN            
            ADC_SoftwareStartConvCmd(ENABLE); //启动下一次ADC转换
