...
硬件资源：
          ...
          7. 系统节拍定时器设置为100ms产生一次中断

演示说明：
          配置IWDT的ARR为127，即IWDT的计时时常为
          （127 + 1）* 15.6ms = 2s，并且溢出后产生中断，不复位。
          系统节拍配置为每 100 ms 产生一个中断，中断复位程序中 LED2闪烁，并喂狗。
          按键KEY2配置为在其下降沿上产生中断。
          ...
          当程序正常运行时，LED2亮，当按下KEY2后，LED2闪烁间隔变为2000ms。
