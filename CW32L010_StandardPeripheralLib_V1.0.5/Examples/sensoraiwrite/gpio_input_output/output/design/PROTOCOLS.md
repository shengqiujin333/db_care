...
| 2 | Read byte 0 | status_byte | Bit[7]=1=bussy, 0=ready; Bit[3]=calibration enable |
...
// Simplified: temp = (raw_temp * 200.0 / 1048576.0 - 50.0) * 10

uint16_t hum_x10 = (uint16_t)(raw_hum * 1000.0 / 1048576.0);  // 0..1000 = 0.0..100.0%
// Simplified: hum = (raw_hum * 100.0 / 1048576.0) * 10
```
...
| 0x01 | CFG2 | Output power, modulation | 0x0X (0dBm OOK/GFSK) |
...
─────────────────────────────────
 ...
 2    0xAA   UID[0:0]  (lower byte of UID word 0)
 3    0xBB   UID[1:1]
 4    0xCC   UID[2:2]
 5    0xDD   UID[3:3]
 6    0xEE   UID[4:4]
...
CRC8 computation for this example:
...
# crc8_maxim(bytes([0xAA, 0x55, 0xAA, 0xBB, 0xCC, 0xDD, 0xEE, 0xFF, 0x00, 0xFE, 0x02, 0x6F, 0x07]))
# => 0x?? (depends on actual CRC computation; firmware shall compute at runtime)
```
