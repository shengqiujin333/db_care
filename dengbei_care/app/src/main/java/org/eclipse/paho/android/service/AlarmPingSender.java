...
//		service.registerReceiver(alarmReceiver, new IntentFilter(action));
		if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.O) {
			service.registerReceiver(alarmReceiver, new IntentFilter(action), RECEIVER_EXPORTED); // 或 RECEIVER_NOT_EXPORTED
		} else {
			service.registerReceiver(alarmReceiver, new IntentFilter(action)); // 老版本无需 flags
		}
...
//				action), PendingIntent.FLAG_UPDATE_CURRENT);
		...
		if (android.os.Build.VERSION.SDK_INT >= android.os.Build.VERSION_CODES.S) {
			alarmIntent = PendingIntent.getBroadcast(service, 0, new Intent(
					action), PendingIntent.FLAG_IMMUTABLE | PendingIntent.FLAG_UPDATE_CURRENT);
		} else {
			alarmIntent = PendingIntent.getBroadcast(service, 0, new Intent(
					action), PendingIntent.FLAG_UPDATE_CURRENT);
		}
